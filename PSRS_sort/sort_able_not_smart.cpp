...
void sort_num(int*num,int s,int e){
    int i,j;
    for(i = s;i < e;i++){
        for(j = i;j > s;j--){
            if(num[j-1]>num[j]){
...
double sort(int*num,int N){
    ...
    send_begin[p-1] = send_begin[p-2] + send_length[p-2];
    send_length[p-1] = e - send_begin[p-1];


    //这一步是把这些数据段播送给对应的进程，并从各个线程中收到自己要进行处理的数据段，这里的part_length是一个进程得到的每一段的长度，要算出给到一个进程的总长度
    ...
    //这一步就得到了收到的段的具体的值，存在concat_num中，这里会初始化两个新的数组，因为这个total_length不能够提前算出，不能和前面的写到一起
